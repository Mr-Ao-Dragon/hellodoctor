...
export default {
  ...
  methods: {
    goto(item) {
      uni.navigateTo({
        url: `/pages/info_reserve?by=doc&id=${item.id}&doc_name=${item.doc_name}&doc_avatar=${item.doc_avatar}&doc_id=${item.doc_id}&name=${item.name}&mobile=${item.mobile}&time=${item.time}&status=${item.status}`,
      })
    },
    getStatus(status){
